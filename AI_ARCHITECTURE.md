...
*   **[26/09/2026 - 18:00]** - Auditoría completa + refactor de seguridad y features. `backend/api.py`: FIX dotenv (carga `.env` raíz + `backend/.env`; `GROQ_API_KEY` ya se carga), FIX path-traversal en `/files/`, FIX condición de carrera `temp_exec.bat` (nombres únicos), rutas hardcodeadas → `sys.executable`/`BACKEND_DIR`, eliminado `FUSION_CHEAT_SHEET` muerto. `las_preview.py` + `tif_preview.py`: previews a PNG (fix incompat matplotlib/Pillow en JPEG) y eliminado plotly-3D muerto. NUEVO: autenticación Google OAuth (`/api/auth/google|me|logout`), SQLite `backend/fusion.db` (usuarios + sesiones + proyectos por usuario), historial persistente (`/api/projects` CRUD), Drive por usuario (`/api/drive/status|upload`; dueño→rclone, otros→Drive API con token de cliente), `/api/config`. Frontend `App.jsx`: login Google (GIS), sync servidor vs localStorage, saludo personalizado "Manos a la obra, {nombre}", modelo por defecto DeepSeek, fix mojibake, PWA (manifest/sw/icons/viewport-fit/theme-color). `.gitignore` creado, `backend/cleanup.sh` + cron horario (retención 24h de binarios). Limpieza de espacio: desinstalados plotly/groq, borrados test.jpg/index.html-raíz/Catalog/logs/App.css/assets y `-max_slope.las` (80MB).
